...
Version **0.2.0b2** is the current beta release and is published on
[TestPyPI](https://test.pypi.org/project/Products.SQLUserWizard/0.2.0b2/).
...
confirmation work, and [release notes](docs/release-0.2.0b2.md) before upgrading.
...
to work with 0.2.0b2.** They have not been dropped merely because every live test
...
checked-out source version. For the exact release, select the `v0.2.0b2` tag:

```bash
git clone https://github.com/fixader/Products.SQLUserWizard.git
cd Products.SQLUserWizard
git checkout v0.2.0b2
pip install -e .
```
...
```bash
pip install "Products.SQLUserWizard @ git+https://github.com/fixader/Products.SQLUserWizard.git@v0.2.0b2"
```
